...
const Navbar = ({ socket }) => {
  ...
  const checkBlocked = async (user1, user2) => {
    const blocked1 = await likesService.isUnlikedBy(user1, user2)
    const blocked2 = await likesService.isUnlikedBy(user2, user1)
  ...
  useEffect(() => {
    ...
    socket.on('show-msg-notification', async (data) => {
      const blocked = await checkBlocked(data.user1, data.user2)
      if (!blocked) {
        if (loggedInUser.id === data.user2) {
